...
description: Explores codebase and writes structured analysis documents to .planning/codebase/.
tools:
  read: true
  bash: true
  grep: true
...
<why_this_matters>
**These documents are consumed by `/load-plan-context` and future planning/execution:**
...
Load the matching skill for templates and rules:
- `tech` -> `codebase-map-tech`
- `arch` -> `codebase-map-arch`
- `quality` -> `codebase-map-quality`
- `concerns` -> `codebase-map-concerns`
</step>
...
Load the matching skill and write documents to `.planning/codebase/` using its templates.
